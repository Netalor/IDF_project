...
from src.visualization.visualize import statistic_plot
...
def get_likui_df(df):
    ...
    logger.info(f'sl_df.shape: {sl_df.shape}')
    logger.info(f'sl_df cols: {sl_df.columns}')
...
def get_bagrut_df(df):
    ...
    logger.info(f'bagrut_df.shape: {bagrut_df.shape}')
    logger.info(f'bagrut_df cols: {bagrut_df.columns}')
...
def load_manila_data(path):
    ...
    logger.info(f'manila_data.shape: {manila_data.shape}')
...
def load_students_data(path):
    ...
    logger.info(f'students_data.shape: {students_data.shape}')
...
def load_stat_socio(path):
    ...
    logger.info(f'stat_socio.shape: {stat_socio.shape}')
...
def load_yeshuv_napa(path):
    ...
    logger.info(f'yeshuv_napa.shape: {yeshuv_napa.shape}')
...
def filter_df_by_target_cond(df: pd.DataFrame, name):
   ...
   # y = 'אשכול תפקידי מקצועות המחשב_אשכול מקצועות המחשב'
        ...
        logger.info('no condition for y - folder name')
        filtered_data = None
        exit()
    logger.info(f'df.shape before filtering: {df.shape}')
    logger.info(f'df_filtered.shape: {filtered_data.shape}')
    logger.info(f'ratio of the data after filtering: {filtered_data.shape[0] / df.shape[0]}')

    filtered_data['original_target'].describe().to_csv(f"output/{folder_name}/analytics/target_desc.csv")
    import plotly.express as px
    filtered_data['original_target'].fillna(0, inplace=True)
...
def load_data(path):
    ...
    if include_bagrut:
        ...
        logger.info(f'dbagrut_df shape: {bagrut_df.shape}')

    if include_likui:
        likui_df, likui_cols = get_likui_df(df)
        logger.info(f'likui_df shape: {likui_df.shape}')

    # merge into one dataset
    df_merged = df.join(students_data.set_index('encoded_mi'), how='left')
    logger.info(f'df+students_data shape: {df_merged.shape}')
    # df_merged['city_code']
    df_merged = df_merged.join(stat_socio.set_index('encoded_mi'), how='left')
    logger.info(f'df+students_data+stat_socio shape: {df_merged.shape}')

    df_merged = df_merged.merge(yeshuv_napa, how='left', left_on='yeshuv_code', right_on='city_code')
    logger.info(f'df+students_data+stat_socio+yeshuv_napa shape: {df_merged.shape}')
    # data['city_code']
    if include_bagrut:
        df_merged = df_merged.merge(bagrut_df, how='left', left_index=True, right_index=True)
        logger.info(f'df+students_data+stat_socio+yeshuv_napa+bagrut_df shape: {df_merged.shape}')

    if include_likui:
        df_merged = df_merged.merge(likui_df, how='left', left_index=True, right_index=True)
        logger.info(f'df+students_data+stat_socio+yeshuv_napa+bagrut_df+likui_df shape: {df_merged.shape}')
    logger.info(f'df_merged.columns: {df_merged.columns.to_list()}')
...
def filter_data(df_merged, threshold=0.5):
    ...
    logger.info(f'null_data_results: {null_data_results.head(10)}')

    not_nan_columns = get_non_nan_cols(null_data_results, threshold)
    logger.info(f'len of not_nan_columns: {len(not_nan_columns)}')

    df_clean = df_filtered[not_nan_columns]
    logger.info(f'df_clean.shape: {df_clean.shape}')
    # remove na
    logger.info(f'df_clean.shape: {df_clean.shape}')
    return df_clean
